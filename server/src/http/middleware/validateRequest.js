...
function parseTaskPatchBody(body) {
  ...
  if (body?.sort_order !== undefined) {
    ...
    if (!Number.isInteger(n) || n < 0) throw new ValidationError({ sort_order: "is invalid" });
    patch.sort_order = n;
  }
...
function parseSseTokenQuery(query) {
  const fields = {};
  const token = requiredField(fields, "token", query?.token, asString);
  failIf(fields);
  return { token };
}

function parseTaskReorderBody(body) {
  const c = body?.columns;
  if (!c || typeof c !== "object" || Array.isArray(c)) {
    throw new ValidationError({ columns: "is required" });
  }
  const out = { columns: { todo: [], in_progress: [], done: [] } };
  for (const status of ["todo", "in_progress", "done"]) {
    const arr = c[status];
    if (!Array.isArray(arr)) {
      throw new ValidationError({ columns: `${status} must be an array of task ids` });
    }
    for (const raw of arr) {
      const id = uuidLike(raw);
      if (!id) throw new ValidationError({ columns: "invalid task id" });
      out.columns[status].push(id);
    }
  }
  return out;
}
